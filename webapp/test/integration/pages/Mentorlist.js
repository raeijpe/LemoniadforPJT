sap.ui.define([
		...
		"com/sap/mentors/lemonaid/projectteam/test/integration/pages/shareOptions"
	], function(Opa5, AggregationLengthEquals, AggregationFilled, PropertyStrictEquals, Common, shareOptions) {
		"use strict";

		var sViewName = "Mentorlist",
			sTableId = "table",
			...
			onTheMentorlistPage : {
				...
				actions : jQuery.extend({
						...
						return this.waitFor(createWaitForItemAtPosition({
							...
							success : function (oTableItem) {
								...
								this.getContext().currentItem = {
									...
									name: oBindingContext.getProperty("Fax")
								};
							}
						}));
					},
					...
					iSearchForTheFirstObject: function() {
						...
						this.waitFor({
							...
							errorMessage: "Failed to find search field in Mentorlist view.'"
						});
				...
				assertions: jQuery.extend({
					...
					theTableShouldHaveAllEntries : function () {
						...
						// retrieve all CompanySet to be able to check for the total amount
							...
							success : function (oTable) {
								...
								this.waitFor({
									...
									matchers : function (oPage) {
										var sExpectedText = oPage.getModel("i18n").getResourceBundle().getText("MentorlistTableTitleCount", [iObjectCount]);
					...
					iShouldSeeTheMentorlistViewsBusyIndicator : function () {
						return this.waitFor({
							id : "page",
							viewName : sViewName,
							success : function (oPage) {
								Opa5.assert.ok(oPage.getParent().getBusy(), "The Mentorlist view is busy");
							},
							errorMessage : "The Mentorlist view is not busy"
						});
					},

					iShouldSeeTheMentorlistTableBusyIndicator : function () {
						return this.waitFor({
							id : "table",
							viewName : sViewName,
							matchers : function (oTable) {
							...
							success : function () {
								Opa5.assert.ok(true, "The Mentorlist table is busy");
							},
							errorMessage : "The Mentorlist table is not busy"
						});
					},
					...
					iShouldSeeTheNoDataTextForNoSearchResults : function () {
						return this.waitFor({
							...
							viewName : sViewName,
							success : function (oTable) {
								Opa5.assert.strictEqual(oTable.getNoDataText(), oTable.getModel("i18n").getProperty("MentorlistNoDataWithSearchText"), "the table should show the no data text for search");
							},
